...
7. [Open5GS Core Commands](#open5gs-core-commands)
...
> **Note:** These parameters give srsRAN finer control over latency, which is why it can achieve lower end-to-end latency than OAI with similar TDD configurations.

---

## Open5GS Core Commands

### Quick Start from Source Build

```bash
cd /home/venu/open5gs
...
```
/home/venu/open5gs/install/etc/open5gs/
├── amf.yaml
├── smf.yaml
├── upf.yaml
└── ...
...
### For OAI with B210 USRP

1. **Use 2ms TDD periodicity** (`dl_UL_TransmissionPeriodicity = 4`)
2. **Set `sl_ahead = 4`** for stable timing with USB3
3. **Force continuous UL** with `ulsch_max_frame_inactivity = 0`
4. **Reduce LDPC iterations** to 4 for faster decoding
5. **Use 8 PUSCH processing threads** for parallel decoding

### For Minimum Latency

| Priority | Action | Expected Impact |
|----------|--------|-----------------|
| 1 | Use 2ms TDD period | -3ms latency |
| 2 | Enable continuous UL scheduling | -2ms variable delay |
| 3 | Reduce LDPC iterations | -0.5ms processing |
| 4 | Optimize RACH timing | -5ms initial access |

### When to Use srsRAN Instead

Consider srsRAN Low Latency fork if:
- You need sub-10ms end-to-end latency
- K1/K2 timing optimization is critical
- You need SR-free uplink access
- Processing delay control is required
...
| Component | Path |
|-----------|------|
| OAI gNB Config | `~/openairinterface5g/targets/PROJECTS/GENERIC-NR-5GC/CONF/` |
| Open5GS Executables | `/home/venu/open5gs/install/bin/` |
| Open5GS Configs | `/home/venu/open5gs/install/etc/open5gs/` |
| srsRAN Low Latency Config | `gnb_low_latency.yml` (from GitHub fork) |

---

*Report generated from OAI URLLC configuration analysis session*
